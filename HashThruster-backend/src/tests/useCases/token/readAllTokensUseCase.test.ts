...
const { createTokenUseCase, readAllTokensUseCase } = useCases;
...
describe('Read all tokens', () => {
  ...
  const readTokens = readAllTokensUseCase(dependencies).execute;
  ...
  test('Should return 200 if good informations', async () => {
    ...
    expect(allTokens.content.tokens).toBeDefined();
    expect(allTokens.content.tokens.length).toBeGreaterThan(0);

    // Vérifier que le premier projet a le même nom que celui ajouté
    expect(allTokens.content.tokens[0].projectName).toEqual(addedToken.projectName);
  });
});
