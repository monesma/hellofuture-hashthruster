...
const { createTokenUseCase, readOneWalletFromTokenUseCase } = useCases;
...
describe('Read One token', () => {
  ...
  test('Should return 200 if good informations', async () => {
    ...
    expect(oneToken.content.walletInfo).toBeDefined();
    expect(oneToken.content.walletInfo.walletAccountId).toEqual(addedToken.walletAccountId);
  });
});