...
const { createTokenUseCase, deleteWalletFromTokenUseCase } = useCases;
...
describe('Delete wallet in an existing token', () => {
  const dependencies = {
    ...useCases,
    ...repositories,
  };
  ...
  const deleteWalletFromToken = deleteWalletFromTokenUseCase(dependencies).execute;

  beforeEach(async () => {
    addedToken = generateTestTokenData();
  });

  test('Should delete Wallet from token selected and return 200 if the token exists', async () => {
    const createResponse = await createToken(addedToken);

    const tokenId = createResponse.content.createdToken._id;
    const deleteResponse = await deleteWalletFromToken(tokenId);

    expect(deleteResponse.status).toBe(200);
    expect(deleteResponse.content.updatedToken.walletAccountId).toBe('deleted');
  });
});
