...
const { createProjectUseCase, readOneProjectUseCase } = useCases;
...
describe('Read One projects', () => {
  ...
  test('Should return 200 if good informations', async () => {
    ...
    const oneProject = await readProject(response.content.createdProject._id);
    expect(oneProject.status).toBe(200);
    expect(oneProject.content.project).toBeDefined();
    expect(oneProject.content.project.projectName).toEqual(addedProject.projectName);
  });
});